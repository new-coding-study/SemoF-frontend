...
import {callMeetingNListEmpAPI} from "../../apis/MeetingAPICalls";
...
function NStatusForEmp () {
    ...
    const AllNStatus = useSelector(state => state.meetingReportReducer.nForEmp);
    ...
    useEffect (() =>{
        dispatch(callMeetingNListEmpAPI({
                ...
                <tbody>
                    {Array.isArray(NStatusLists) && NStatusLists.map((n) => (
                        <tr
                            ...
                            onClick={()=>
                                onClickDetailMeeting(n.meetingReportCode)
                            }>
                            <td>{n.rowNum}</td>
