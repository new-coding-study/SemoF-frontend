...
export const callGetMainEmpBirthAPI = () => {
  console.log("[EmployeeAPICalls] callGetMainEmpBirthAPI Call");
  ...
  return async (dispatch, getState) => {
    ...
    console.log("[EmployeeAPICalls] callGetMainEmpBirthAPI RESULT : ", result);
    if (result.status === 200) {
      console.log("[EmployeeAPICalls] callGetEmpBirthAPI SUCCESS");
      dispatch({ type: GET_MAIN_EMPLOYEES_BIRTH, payload: result.data });
    }
  };
};
