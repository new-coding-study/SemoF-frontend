...
function Attendance() {
  ...
  const status = useSelector(
    (state) => state.AttendanceReducer.attendanceStatus
  );
  const statusList = useSelector(
    (state) => state.AttendanceReducer.attendanceList
  );
  const putStatus = useSelector(
    (state) => state.AttendanceReducer.updateAttendance
  ); // # 리듀서를 복사해서 같은 곳을 참조하지만 다른 값을 가지게 설정
  ...
  function calculateTime(status) {
    ...
    const hours = String(Math.floor(diffTime / (1000 * 60 * 60)) % 24).padStart(
      2,
      "0"
    );
      ...
      <div style={{ minWidth: "1680px" }}>
        <div className={AttendanceCSS.horizontalDiv}>
          <div className={AttendanceCSS.attendance}>
            <div>
              <span>
                {condition} {newTime.hours}:{newTime.minutes} {newTime.month}/
              ...
              <tbody>
                {Array.isArray(attendanceList) &&
                  attendanceList.map((p) => (
                    <tr key={p.atdNo}>
                      <td>
                        {p.atdDate === null ? "" : p.atdDate.substring(0, 10)} (
                        {
                          week[
                            String(new Date(p.atdDate).getDay()).toUpperCase()
                          ]
                        }
                        )
                      </td>
                      <td>
                        {p.startTime === null ? "" : p.startTime.substring(10)}
                      </td>
                      <td>
                        {p.endTime === null ? "" : p.endTime.substring(10)}
                      </td>
          ...
          >
            ...
            {pageNumber.map((num) => (
              <li key={num} onClick={() => setCurrentPage(num)}>
                <button
                  style={
                    currentPage === num
                      ? { backgroundColor: "#E52E2E", color: "white" }
                      : null
                  }
                  className={AttendanceCSS.pagingBtn}
                >
            ...
            {Array.isArray(attendanceList) && (
              <button
                ...
                onClick={() => setCurrentPage(currentPage + 1)}
                disabled={
                  currentPage === pageInfo.endPage || pageInfo.totalCount === 0
                }
              >
                &gt;
              </button>
